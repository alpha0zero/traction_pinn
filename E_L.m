...
Ne = 20000;  % # of Epochs (1 Epoch contains Tb training batches)
...
while (bi <= Tb) && (costi > Tt)
    ...
    mySzp = mysigmoid(w0*x+b0).*(1-mysigmoid(w0*x+b0));% Dubbel prime -||-
    mySzp_l = mysigmoid(w0+b0).*(1-mysigmoid(w0+b0));
    ...
    % Partial derivatives of dy/dx|x=l w.r.t weights and biases
    dy1dw0 = w1.*mySzp + w1.*(w0).^2.*mySzpp;
    dy1dw1 = w0.*mySzp;
    dy1db0 = w1.*w0.*mySzpp;
    dy1db1 = 0;
